...
async def cpa_session(lumenvox_api,
                      audio_ref: str,
                      chunk_size: int = 4000,
                      audio_format: int = None, sample_rate_hertz: int = None,
                      deployment_id: str = None, operator_id: str = None, correlation_id: str = None,
    ...
    """
    Function to open session, and run test cpa interaction
    ...
    await cpa_streaming_interaction(lumenvox_api, session_stream)
...
async def cpa_streaming_interaction(lumenvox_api, session_stream):
    """
    Function to run test cpa interaction
    ...
    # cpa uses voice activity detection
    ...
    # CPA Settings
    # In this example, we're requiring the CPA to determine outcome in first 3 seconds of audio
    cpa_settings = lumenvox_api.define_cpa_settings(max_time_from_connect_ms=3000)

    # create a cpa interaction with supplied settings
    await lumenvox_api.interaction_create_cpa(session_stream=session_stream, cpa_settings=cpa_settings,
    ...
    interaction_id = r.interaction_create_cpa.interaction_id
